...
def epidemicData():
	...
	sql = "INSERT INTO epidemicdata (date, current_confirmed_count,current_confirmed_incr, confirmed_count, " \
	      "confirmed_incr, overseas_count, overseas_incr, cured_count, cured_incr, dead_count, dead_incr, " \
	      "asymptomatic_count, asymptomatic_incr) VALUES ('%s', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)" % (
...
def epidemicNews():
	...
	try:
		cursor.execute("DELETE FROM epidemicnews")
		db.commit()
	except Error as err:
		print("OS error: {0}".format(err))
		db.rollback()
	...
	for news in jsonData['results']:
		...
		sql = "INSERT INTO epidemicnews (pub_date, title, summary, info_source, source_url) VALUES (%s,'%s','%s','%s','%s')" \
